...
with campaign_pop_raw as (
  ...
  from {{ s('jago_gobiz_data_sharing', 'monthly_gobiz_eligible_merchant_benefit', has_stube=flase) }}
    join {{ r('data_analytics__datamart_base_customer', has_stube=flase) }} customer using(customer_id)
  where
